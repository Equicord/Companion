...
function ExpandableHeader({
  header,
  children,
}: PropsWithChildren<ExpandableHeaderProps>) {
  ...
  return (
    <div>
      ...
      <div style={{ marginLeft: "2rem" }}>{expanded && (children ?? null)}</div>
    </div>
  );
}
        ...
        <ExpandableHeader header={`${name} (${data.length})`}>
          <div style={{ display: "flex", flexDirection: "row", gap: "0.5rem" }}>
            <Button
              ...
              onClick={() => disablePlugin(name)}
            />
            <Button
              label={"Enable Plugin"}
              onClick={() => enablePlugin(name)}
            />
          </div>
                ...
                return (
                  <div className="EvaledPatch">
                    ...
                    >
                      ...
                      <Button
                        label={"Jump"}
                        onClick={() => jumpToPatch(name, e)}
                      />
                      <Button
                        label={"Diff"}
                        onClick={() => diffPatch(e)}
                        disabled={type === "hadNoEffect" || !e.id}
                      />
                      <Button
                        label={"Extract"}
                        onClick={() => extractPatch(e)}
                        disabled={!e.id}
                      />
                    </div>
                    <Codeblock label="Find:">{e.find}</Codeblock>
                    <Codeblock label="Module Number:">
                      {e.id || "------"}
                    </Codeblock>
                    <Codeblock label="Match:">
                      {String(e.replacement[0].match)}
                    </Codeblock>
                    <Codeblock label="Replace:">
                      {String(e.replacement[0].replace)}
                    </Codeblock>
                  </div>
                );
              return (
                <>
                  {!shown.includes(index) && (
                    <div className="EvaledPatch" key={index}>
                      <ExpandableHeader
                        header={<Codeblock label="">{e.find}</Codeblock>}
                      >
                        <div
                          style={{
                            display: "flex",
...
function ReportList({ brokenPatches, name }: ReportListProps) {
  ...
  const list = brokenPatches.reduce(
    (acc, obj) => {
...
export function ReporterSections({ data }: ReporterSectionsProps) {
  ...
  return (
    <div id="reporter">
      <h1>Equicord Reporter Report</h1>
      {failedPatches &&
        Object.entries(failedPatches).map(([key, value]) => {
          if (!value.length) return null;
          return <ReportList brokenPatches={value} name={key} />;
        })}
      {failedWebpack &&
        Object.entries(failedWebpack).map(([k, v]) => {
          if (v.length === 0) return null;
          return <FindList brokenFinds={v} name={k} />;
        })}
      <div id="footer">
        Made by{" "}
        <a rel="noreferrer" target="_blank" href="https://samwich.dev/">
          Samwich
        </a>{" "}
        and{" "}
        <a rel="noreferrer" target="_blank" href="https://sadan.zip">
          Sadan
        </a>
      </div>
    </div>
