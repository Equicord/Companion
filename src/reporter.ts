...
// 3. await results back from equicord
...
export async function startReporter() {
  try {
    if (running) {
      running = false;
      throw new Error(
        "The reporter is currently running, please wait.\nIf you think this is a bug, please report this.",
      );
    }
...
async function getNormalBuildTask() {
  const task = (await tasks.fetchTasks()).filter(
    t =>
      ...
      !t.execution.commandLine?.includes("--companion-test"),
  );
  if (task.length === 0) throw new Error("No build task found");
  if (task.length > 1) throw new Error("More than one task found");
  return task[0];
}
